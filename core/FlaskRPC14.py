...
#        self.app.run(host='0.0.0.0', port=80)
    ...
    def app_run(self):
        # self.app.run(host='0.0.0.0', port=80)
          self.renew_cert()
          self.app.run(host='0.0.0.0', port=443, ssl_context=('/usr/src/cert.pem', '/usr/src/key.pem'))

    def renew_cert(self):
         CERT_PATH = "/usr/src/cert.pem"
         KEY_PATH = "/usr/src/key.pem"
         DAYS_BEFORE_RENEWAL = 5  # Set how many days before expiry you want to renew the certificate

         try:
            # Get the expiration date of the current certificate using subprocess to call openssl
    ...
    # Calculate days left before expiration
            current_date = datetime.now()
            days_left = (expiry_date - current_date).days
            print(f"Certificate expires in {days_left} days.")
  
            # If days left are less than or equal to the renewal threshold, regenerate the certificate
            if days_left <= DAYS_BEFORE_RENEWAL:
                  print("Renewing the SSL certificate...")

                  # Create a new certificate using subprocess to call openssl
                  subprocess.run([
                     'openssl', 'req', '-x509', '-newkey', 'rsa:4096', '-nodes',
                     '-out', CERT_PATH, '-keyout', KEY_PATH, '-days', '365',
                     '-subj', "/C=IN/ST=TELANGANA/L=HYDERABAD/O=BLYK Engineering Services/OU=LEDERBORD/CN=scorelight/emailAddress=scorelightllc@gmail.com"
                  ], check=True)

                  print("New SSL certificate generated, valid for 363 days.")
            else:
                  print(f"SSL certificate is still valid for more than {DAYS_BEFORE_RENEWAL} days. No renewal needed.")

         except subprocess.CalledProcessError as e:
            print(f"Error while checking or renewing certificate: {e}")
...
#               return response
            ...
            try:
              ...
              # expiry_date = datetime.strptime(expiry_date_str, "%b %d %H:%M:%S %Y %Z")
               ...
               if '.' in method:
                  ...
                  while '.' in method:
                    # print(method)
                     # print(obj, obj._dict_)
                     comps = method.split('.')
                     print(comps[0])
                     obj = getattr(obj, comps[0])
                     method = '.'.join(comps[1:])
                      
               # print('END OF WHILE', obj, obj._dict_)
                  ...
                  if method == 'info':
                      ...
                      resp["certificate_expiry"] = expiry_date_str           
        ...
        def get_Current_Gameinfo():
           brightness=''
         # Ensure rootview is properly set up
           if hasattr(self, 'rootView'):
              brightness= self.rootView.__options__.limit_refresh_rate_hz

         # Assuming rootview.__options__ is a valid attribute
           try:
              game_info = {
                    "Id": self.gameid,
                    "brightness": brightness   # or other properties
              }
              return jsonify(game_info)
           except AttributeError as e:
              print(f"Error: {e}")
              return "Error: rootview configuration failed", 500
        ...
        def Set_Reset_Board():
         ...
         # New route for updating the SSL certificate
         

        @app.route('/update-certificate', methods=['POST'])
        def update_certificate():
            CERTIFICATE_PATH = "/usr/src/cert.pem"
            KEY_PATH = "/usr/src/key.pem"
        ...
        # Get the uploaded certificate file
            ...
            if os.path.exists(CERTIFICATE_PATH):
                 os.remove(CERTIFICATE_PATH)  # Delete the old certificate

                     # Save the new certificate
            file.save(CERTIFICATE_PATH)
                     
            return jsonify({'message': 'Certificate updated successfully and old one deleted'}), 200
        

       # @app.route('/renew-cert', methods=['GET']
...
#                      Inning2=int(web.board.HomeServe)
        ...
        def Start_timer():
         ...
         if request.method == 'GET':
            # Retrieve the 'time' parameter from the query string
            data = request.args.get('time')
            print(data)  # Debug print to see the time value
            if data:
                  t = int(data)  # Convert the time value to an integer

            # Check if the timer_event is not already set and then set it
            if not timer_event.is_set():
                  timer_event.set()
                  timer_status = 1
                  print("Timer event set successfully")  # Additional debug statement
                  return "Success", 200  # Return a success message and HTTP 200 status code
         
         # If the timer was already set or the time parameter was not provided
         return "Timer is already running or invalid time parameter", 400
...
#            case 1:
    ...
    def start(self, dataStr=None):
        if self.rootView is None:
            ...
            self.rootView.setBrightness(1000)
        else:
...
#                    print(defHome["R"])
                        ...
                        if int(self.gameinfo["homewinScore"])<10:
                           if len(self.gameinfo["homewinScore"]) == 1:
                        ...
                        if int(self.gameinfo["awaywinScore"])<10:
                           if len(self.gameinfo["awaywinScore"]) == 1:
...
#                        dataStr["awayScore"] = self.gameinfo["awayScore"]
    ...
    def createClock(self, dataStr=None):
        global game_id
        print(game_id)
        t=0
        if self.rootView == None:
...
#        self.setBrightness(self,dataStr)
    ...
    def info(self, dataStr=None):
        ...
        data={"ssid":ssid,"password":password,"version":"V1.2A"}
...
#    GPIO.setmode(GPIO.BCM)
   ...
   # app.run(ssl_context=(CERTIFICATE_PATH, KEY_PATH))
